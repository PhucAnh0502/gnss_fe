...
function EditZonePanel({ zone, editName, setEditName, editRadius, setEditRadius, onCancel, onSuccess, editDrawPoints, isEditDrawing, onStartEditDraw, onUndoEditPoint, onFinishEditDraw, onCancelEditDraw }) {
  ...
  const handleSave = async () => {
    ...
    // Include new polygon if user redrew it
    if (editDrawPoints && editDrawPoints.length >= 3) {
      payload.polygon = latLngsToGeoJSON(editDrawPoints);
    }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {/* Optional: Redraw polygon */}
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1">Polygon (optional)</label>
          {!isEditDrawing ? (
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onStartEditDraw}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-orange-400/30 bg-orange-500/10 text-xs font-medium text-orange-200 hover:bg-orange-500/20 transition-colors"
              >
                <Pencil className="w-3 h-3" />
                Redraw Polygon
              </button>
              {editDrawPoints && editDrawPoints.length >= 3 && (
                <span className="text-xs text-emerald-300">{editDrawPoints.length} vertices (new)</span>
              )}
              {(!editDrawPoints || editDrawPoints.length === 0) && (
                <span className="text-xs text-slate-500">Keep current shape</span>
              )}
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className="text-xs text-orange-300">Click map to draw ({editDrawPoints?.length || 0} points)</span>
              <button
                type="button"
                onClick={onUndoEditPoint}
                disabled={!editDrawPoints || editDrawPoints.length === 0}
                className="p-1 rounded-lg border border-slate-700 text-slate-300 hover:bg-slate-800 disabled:opacity-40 transition-colors"
                title="Undo"
              >
              ...
              <button
                type="button"
                onClick={onFinishEditDraw}
                disabled={!editDrawPoints || editDrawPoints.length < 3}
                className="px-2 py-1 rounded-lg border border-emerald-400/30 bg-emerald-500/10 text-[10px] font-medium text-emerald-200 hover:bg-emerald-500/20 disabled:opacity-40 transition-colors"
              >
                Done
              </button>
              <button
                type="button"
                onClick={onCancelEditDraw}
                className="px-2 py-1 rounded-lg border border-rose-400/30 bg-rose-500/10 text-[10px] font-medium text-rose-200 hover:bg-rose-500/20 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="flex items-center gap-2 pt-2">
          <button
            type="button"
            onClick={handleSave}
            disabled={updateMutation.isPending || isEditDrawing}
...
export function MapTrackingTab({ center, devicePoints, selectedTrack, selectedDeviceCode, isAdmin }) {
  ...
  // Edit drawing state (optional polygon redraw)
  const [isEditDrawing, setIsEditDrawing] = useState(false);
  const [editDrawPoints, setEditDrawPoints] = useState([]);
  ...
  const handleStartEditDraw = useCallback(() => {
    setIsEditDrawing(true);
    setEditDrawPoints([]);
  }, []);

  const handleUndoEditPoint = useCallback(() => {
    setEditDrawPoints((prev) => prev.slice(0, -1));
  }, []);

  const handleFinishEditDraw = useCallback(() => {
    setIsEditDrawing(false);
  }, []);

  const handleCancelEditDraw = useCallback(() => {
    setIsEditDrawing(false);
    setEditDrawPoints([]);
  }, []);
  ...
  return (
    <>
      <MapContainer center={center} zoom={12} scrollWheelZoom className="h-full w-full">
        ...
        {/* Drawing layer */}
        <DrawingLayer drawing={drawing || isEditDrawing} points={drawing ? drawPoints : editDrawPoints} onAddPoint={drawing ? handleAddPoint : (point) => setEditDrawPoints((prev) => [...prev, point])} />
      </MapContainer>
      ...
      <AnimatePresence>
        {editingZone && (
          <EditZonePanel
            ...
            onSuccess={() => { setEditingZone(null); setIsEditDrawing(false); setEditDrawPoints([]); }}
            editDrawPoints={editDrawPoints}
            isEditDrawing={isEditDrawing}
            onStartEditDraw={handleStartEditDraw}
            onUndoEditPoint={handleUndoEditPoint}
            onFinishEditDraw={handleFinishEditDraw}
            onCancelEditDraw={handleCancelEditDraw}
          />
        )}
      </AnimatePresence>
    </>
